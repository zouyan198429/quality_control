...
//        }
        ...
        CommonDB::doTransactionFun(function() use(&$saveData, &$company_id, &$id, &$operate_staff_id, &$modifAddOprate, &$operate_staff_id_history, &$modelObj, &$isModify
            ...
            // 发票配置购买方id
            if(isset($saveData['invoice_buyer_id']) && is_numeric($saveData['invoice_buyer_id']) && $saveData['invoice_buyer_id'] > 0){
                $tem_invoice_buyer_id = $saveData['invoice_buyer_id'];
                $saveData['invoice_buyer_id_history'] = InvoiceBuyerDBBusiness::getIdHistory($tem_invoice_buyer_id);
            }

            // 发票配置销售方id
            if(isset($saveData['invoice_seller_id']) && is_numeric($saveData['invoice_seller_id']) && $saveData['invoice_seller_id'] > 0){
                $tem_invoice_seller_id = $saveData['invoice_seller_id'];
                $saveData['invoice_seller_id_history'] = InvoiceSellerDBBusiness::getIdHistory($tem_invoice_seller_id);
            }
